...
const BldgInputsContainer = (props) => {
    ...
    return (
        <div>
            <div>
                {props.building.areas.map((e, i) => <BldgTypeInfoBox key={i} index={e.index} type={e.type} area={e.area} />)}
                <Button onClick={handleAddBuildingType}>+</Button>
            </div>

            <div>
                {Object.keys(fueltypes).map((e, i) => {
                    return <div key={i}><NumberInput changeCallback={(v) => handleUtilityChange(v.target.value, e)} label={fueltypes[e]} /></div>
                })}
